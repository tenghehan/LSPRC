...
from models.fusion_block import fusion_block

class CAS_Fusion_ResNet34(nn.Module):
    def __init__(
            self,
            ratio,
            is_global: list
    ):
        super(CAS_Fusion_ResNet34, self).__init__()
        ...
        self.fusion_block_a = fusion_block(attr_num=self.attr_num_a)
        self.fusion_block_b = fusion_block(attr_num=self.attr_num_b)
    ...
    def forward(self, x):
        ...
        x_a_fusion = self.fusion_block_a(x_a_1, x_a_2, x_a_3, x_a_4)
        x_b_fusion = self.fusion_block_b(x_b_1, x_b_2, x_b_3, x_b_4)
        x_fusion = torch.zeros(x_a.shape[0], self.attr_num_a + self.attr_num_b)
        x_fusion = self.combine_a_b(x_a_fusion, x_b_fusion, x_fusion)

        x_main = self.classification(x_a=x_a, x_b=x_b)
...
if __name__ == "__main__":
    model = CAS_Fusion_ResNet34(32, [True] * 11 + [False] * 35 + [True] * 9)
    model.eval()
